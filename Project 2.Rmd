...
#### Full Model
```{r}
testlm <- lm(price ~view + bedrooms + bathrooms + sqft_living + sqft_lot + waterfront + condition + grade + sqft_above + sqft_basement + yr_built + yr_renovated + sqft_living15 + sqft_lot15 + lat + long + floors, data = house)
summary(testlm)
```

#### Adjusted Linear Model
```{r}
testlm2 <- lm(price ~bedrooms + bathrooms + sqft_living + yr_built + yr_renovated + sqft_living15 + lat + long + floors, data = house)
summary(testlm2)
```
...
model <- train(price ~ bedrooms + bathrooms + floors + grade + view + condition + waterfront, data = house, method = "lm", trControl = train.control)
...
```{r,warning=F}
set.seed(123)
model4 <- train(price ~ bedrooms + bathrooms + floors + grade + view + condition + waterfront + sqft_living + sqft_lot + sqft_above + sqft_basement + yr_built + yr_renovated + zipcode, data = house, method = "lm",
               trControl = train.control)
# Summarize the results
print(model4)
```
...
```{r}
modelcom <- matrix(c(231169,0.602,152269, 260958,0.494,173416, 362116,0.024,231970, 208760,0.675,135347, 191991,0.726,119763, 191973,0.726,119731),ncol=3,byrow=TRUE)
